...
simple_Y = torch.tensor(Y[:, 0], device=device).reshape(-1, 1)
...
iid_train_steps = 200
iid_lr = 0.05
iid_blogistic, iid_blogistic_params = train_blogistic(train_Y.flatten(), dof, iid_lr, iid_train_steps, allow_skew=True, device=device)

lr = 1e-3
num_steps = 5000
model, train_losses, dev_losses = train_simple_nn(train_X, train_Y, dev_X, dev_Y, lr, num_steps, device=device)
torch.save(model.state_dict(), "simple_nn_model16.pth")
...
# load the model
model = SimpleNN().to(device)
model.load_state_dict(torch.load("simple_nn_model16.pth"))
