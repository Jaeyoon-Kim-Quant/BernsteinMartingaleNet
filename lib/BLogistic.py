...
class BLogistic:
    ...
    def _initialize_consts(self, degree):
        ...
        mus = torch.tensor(mus, dtype=torch.float64, device=self.device)  # integral of lg(x/(1-x)) x ^ n
        ...
        bernstein_to_standard_matrix_torch = torch.tensor(bernstein_to_standard_matrix, dtype=DT, device=self.device)
