...
class SkewedBLogistic:
    ...
    def logpdf_vectorized(self, xs, coeffs, raw_scale, raw_skewness):
        ...
        #bernstein_poly = u_p * u_m * self.comb
        #log_u_p = torch.log(Fx) * powers.reshape(1, -1)
        log_u_p = -skewness *torch.nn.functional.softplus(-shifted_xs) * powers.reshape(1, -1)
        log_u_m = -torch.nn.functional.softplus(shifted_xs) * reversed_powers.reshape(1, -1)

        print("diff", torch.norm(log_u_p - torch.log(u_p)), torch.norm(log_u_m - torch.log(u_m)))
        ...
        if torch.isnan(log_poly).any():
            print("log_u_p", log_u_p, torch.isnan(log_u_p).any())
            print("log_u_m", log_u_m, torch.isnan(log_u_m).any())
            print("comb", self.comb)
            print("Fx", Fx, torch.isnan(Fx).any())
            print("powers", powers)
            print("reversed_powers", reversed_powers)
            print("normalized_coeffs", normalized_coeffs, torch.isnan(normalized_coeffs).any())
            exit()
